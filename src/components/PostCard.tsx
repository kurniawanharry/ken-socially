...
function PostCard({ post, dbUserId }: { post: Post; dbUserId: string | null }) {
  ...
  const [hasLiked, setHasLiked] = useState(
    post.likes.some((user) => user.userId === dbUserId)
  );
  const [optimisticLikes, setOptimisticLiked] = useState(post._count.likes);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      setOptimisticLiked(post._count.likes);
  ...
  const handleDeletePost = async () => {
    ...
    try {
      ...
      if (result?.success) toast.success("Success delete post");
